...
// Stored client IDs are untrusted, including IDs imported from older backups.
// Filesystem names have no relationship to those IDs.
const materializedPageFilePath = (tempDir: string, _pageId: string) =>
	path.join(tempDir, 'pages', `${randomUUID()}.md`);

const writeMaterializedPageFile = async (
...
const loadProjectWorkspaceState = async (
	...
	const activeBoardId =
		typeof scope?.activeBoardId === 'string' &&
		boards.some((board) => board.id === scope.activeBoardId)
			? scope.activeBoardId
			: boards[0].id;
	const activePageId =
	...
	const board = mapBoardRow(
		activeBoardRow,
		columns.filter((column) => column.board_id === activeBoardRow.id),
		tasks.filter((task) => task.board_id === activeBoardRow.id && !task.deleted_at)
	);
...
export const materializeWorkspace = async (
	...
	const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'kainbu-workspace-'));
...
export const deriveKanbanOps = (original: KanbanData, next: KanbanData): KanbanPatchOperation[] => {
...
const resolveKanbanScope = (
	ops: KanbanPatchOperation[],
	safety: AiProposalSafety
): AiKanbanProposal['scope'] => {
	if (
...
const buildScratchpadStateFromPages = (pages: MaterializedPage[], activePadId: string) => ({
	activePadId: pages.some((page) => page.id === activePadId)
		? activePadId
		: pages[0]?.id || activePadId,
	pads: pages.map((page) => ({
		id: page.id,
		name: page.name,
		content: page.content
