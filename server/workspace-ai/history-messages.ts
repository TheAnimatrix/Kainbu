...
/** Keep saved/undone outcomes in model context without sending undo snapshots. */
export const formatAssistantHistoryText = (message: ChatMessage): string => {
	const changes = (message.appliedProposalChanges || []).map((change) => {
		const status = change.error
			? `${change.status}, needs attention: ${change.error}`
			: change.status;
		return `Workspace change ${status}: ${change.summary}`;
	});
	return [message.text, ...changes].filter(Boolean).join('\n');
};

export const buildHistoryMessages = (history: ChatMessage[]): OpenRouterMessage[] =>
	history.map((message) => {
		if (message.role === 'assistant') {
			return { role: 'assistant', content: formatAssistantHistoryText(message) };
		}
